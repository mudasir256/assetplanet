...
import AssetListing from "../asset-listing/Asset-Listing";
import swal from "sweetalert";
const { Option } = Select;

const AllocationSubform = () => {
  let tabs2;
  const AllTabs = {
    "ASSET INFORMATION": {
    ...
    "QUALIFIED ACCOUNT TYPE QUESTIONS": {
      name: "QUALIFIED ACCOUNT TYPE QUESTIONS",
      enabled: false,
      priority: 2,
    },
    "CONTRIBUTIONS OR DISTRIBUTIONS": {
      name: "CONTRIBUTIONS OR DISTRIBUTIONS",
      enabled: false,
      priority: 3,
    },
  ...
  const [activeTabName, setActiveTabName] = useState("ASSET INFORMATION");
      ...
      {tabs.length > 0 && (
        ...
        >
          <Button
            ...
            disabled={activeTabName == "ASSET INFORMATION"}
          >
            Previous
          </Button>
          {activeTabName !== "TAX AND LIQUIDITY" ? (
            <Button
  ...
  return (
    <>
      <div style={{ textAlign: "center", margin: " 0 3rem" }}>
        <h1>{title}</h1>
        <h5>{title && title==="SELLING ASSET COSTS" && "These fields should only be used if the Asset is sold or being sold. If that does not apply here, please press Next on the bottom of this page."}</h5>
      </div>
      <Styles.FormDiv>
...
export default AllocationSubform;
